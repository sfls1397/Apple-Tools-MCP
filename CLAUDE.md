...
The safety contract shared by every write: `dry_run` previews and never executes; deletes and multi-recipient sends require `confirm`; recipients, ids, bodies, and subjects are validated rather than guessed; success and failure lines name the action and target, and `scrubValues()` keeps caller-supplied bodies out of error text. Planned / dry_run / confirm-blocked results use `plannedWriteResult()` (`ok: false`, `planned: true`, `delivered: false`) so MCP `isError` cannot be read as delivered.
...
One module per data source. Each exports pure `build*Script()` builders (unit-tested without macOS) plus the tool functions. Mail addresses messages by RFC822 Message-ID (resolvable from an `.emlx` `file_path`); `mail_send` / `mail_draft` (plain and html) `make new outgoing message` without AppleScript `content`/`html content` (Ventura+ FB11734014 cite-wraps those setters; Mail `mailto` does not return an outgoing message on Mini/MacBook, -2753) and fill the body via the Mini Probe 3 path: exact subject title on the compose window, first `AXWebArea` from System Events `entire contents` of that window, `set focused of` that element to true, prove focus by re-reading `AXFocusedUIElement` (role `AXWebArea` plus matching position and size), then Cmd-A + clipboard paste. Do not deliver with Mail `content` / `html content`, with `set value`, or with `AXUIElementSetAttributeValue` (2.1.4 `atmAxHitFill` returned OK on an empty AXValue, so paste never ran). Do not `click at` body coordinates (Probe 3: the click can hit another Space or app). `atmTopElemBits` / `atmTopElemExactTitle` must wrap `value of attribute "AXTitle"` inside `tell application "System Events"` (2.1.4 Mini osacompile **-2741** on the bare read). Window find is exact title (`windows` name, with the top-level UI-element exact-title fallback). The body walk is `entire contents` of that one window, not a process-wide tree walk. Apple events are `with timeout`; hung osascript is SIGKILL'd. If the write-bridge daemon cannot drive System Events, return `unsupported: true` so stdio falls back in-process. Fail-closed: BODY_FOCUS_FAILED if the window or AXWebArea is missing; BODY_FOCUS_UNPROVEN if focus proof mismatches (draft deleted, nothing sent); BODY_PASTE_MISDIRECTED when To/Cc/Bcc + subject checks fail. The body needle runs only when the focused element is the proven body, so a To: `U+FFFC` chip is not a false misdirect. `mail_reply` / `mail_forward` still prepend onto the quoted original; a real `mail_send` / `mail_reply` / `mail_forward` is not success until Sent or Outbox verify hits (AppleScript `send` returning is not enough); after a hang the tool verifies the same way (In-Reply-To or exact `Re:` + original subject for reply; intended recipient plus exact Fwd: subject or original Message-ID in a forwarded body for forward — never the original itself or an unrelated Fwd: that only shares a To; **To + subject**, and Message-ID when available, for compose — never subject alone) and returns success if Mail already delivered or the message is still in Outbox — `ETIMEDOUT` / `-1712` is a find/reply/open/send timeout (including before send), never `MAIL_TCC_GUIDANCE`; only `-1743` / `-10004` / deny text is a hard Mail deny; clients must Sent-check before retrying a timed-out send. Planned / `dry_run` / confirm-blocked writes return `ok: false` + `planned: true` so MCP `isError` cannot be mistaken for delivered. Messages verifies a `chat_id` against `chat.db` before sending and counts participants to detect group chats; Calendar addresses events by iCal UID and builds RRULEs from an allowlisted grammar; non-recurring `calendar_add` **must** create via EventKit (`via: EventKit` + `eventkit_id` from the in-memory saved event — writeOnly cannot re-query) and does not fall back to Calendar.app; the write-bridge daemon keeps an EventKit session that caches the `EKEvent` so `calendar_edit` / `calendar_remove` can save/remove without `eventWithIdentifier`; ETIMEDOUT is `timeout` not TCC; Contacts addresses people by Contacts.app person id and never writes the AddressBook database directly.
...
Reads and writes hit different gates and must not be conflated. Contacts and Calendar **reads** are sqlite against `AddressBook-v22.abcddb` / `Calendar.sqlitedb` and need only Full Disk Access, so an `EPERM` there is an FDA/attribution problem. **Writes** go through Mail.app, Messages.app, Contacts.app (`CNContactStore`), and Calendar.app (EventKit). Contacts and Calendar writes are also gated by the AddressBook and calendars privacy classes. Claude.app ships with neither `com.apple.security.personal-information.addressbook` nor `…​.calendars` (verified by codesign; only location and photos-library are present), so Contacts and Calendar CRUD under Claude Desktop are host limitations. The ship gate is the Node host with FDA, reached from any client through the bridge. Mail / Messages / Contacts / Calendar / System Events **writes** are granted by Allowing **node** in System Settings → Privacy & Security → **Automation** (Apple Events to Mail.app / Messages.app / Contacts.app / Calendar.app / System Events) while the LaunchAgent owns node — not by adding node via + in the Contacts or Calendars privacy lists (those panes often have no Add button). Contacts or Calendar being allowed does not grant Mail. `apple-tools-mcp permissions` is the one-shot first-install/upgrade probe for those grants, including System Events process / GUI scripting (not a soft `get name`) for the compose body-fill path. A sticky Don't Allow for node → System Events reports missing and exits non-zero until Automation → System Events is on and `permissions` is re-run. `mail_send` `dry_run` never talks to Mail; a hang on real compose is TCC / Automation denied, not “app not available”. `scripts/smoke-writes.js` proves Contacts, Calendar, a live Mail compose (`make new outgoing message`), and a live Messages account lookup. `--apply` fails closed if any of those four Automation grants is missing.

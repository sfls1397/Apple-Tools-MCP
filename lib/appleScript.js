 ...
 * A hang or timeout on compose (probe / draft / `make new outgoing message`)
 * is that deny — not "Mail.app missing". dry_run never sends Apple events
 * to Mail, so it cannot detect the grant.
 *
 * A hang *after* `send` / `reply` / `forward` is classified separately
 * (`MAIL_SEND_TIMEOUT_GUIDANCE`): Mail may already have delivered.
 */
...
/**
 * Send/reply/forward AppleScript hung (ETIMEDOUT / -1712) after Mail may
 * already have put the message in Sent. That is not -1743 / -10004.
 * Clients must Sent-check before retrying — a retry of a delivered send
 * creates a second copy. This is not a silent TCC grant.
 */
export const MAIL_SEND_TIMEOUT_GUIDANCE =
  "Mail send timed out waiting for AppleScript (ETIMEDOUT / -1712 / AppleEvent timed out) — this is a find/reply/send hang, not a TCC / Automation deny. " +

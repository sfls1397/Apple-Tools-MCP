...
import { runAppleScript, asString, CONTACTS_TCC_GUIDANCE } from "./appleScript.js";
import {
  planWrite,
  normalizeList,
  isEmailAddress,
...
function failure(action, summary, result, secrets = []) {
  ...
  if (result.kind === "app_unavailable") {
    return `${action} failed — attempted to ${summary}. Contacts.app is not available on this host.`;
  }
...
export function contactsRemove(args = {}) {
  ...
  const result = runAppleScript(buildRemoveContactScript(contactId), { timeout: 60000 });
  if (!result.ok) return { ok: false, message: failure(action, summary, result) };

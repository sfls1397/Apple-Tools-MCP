...
export const CONTACTS_READY_ATTEMPTS = 8;
export const CONTACTS_READY_INTERVAL_MS = 250;
...
export function buildContactsReadyScript() {
  ...
  if running then return "READY"
end tell
return "NOT_READY"`;
}

export function contactsAppIsReady(output) {
  return String(output || "").trim().toUpperCase() === "READY";
}
 ...
 * instead of auto-launching. Launch, then poll `running` before CRUD.
 */
export function ensureContactsAppReady({
  ...
  const launch = run(buildContactsLaunchScript(), { timeout: 15000, appName: "Contacts" });
  if (!launch.ok && (launch.kind === "tcc" || launch.kind === "timeout")) {
    return launch;
  }
  ...
  for (let i = 0; i < tries; i++) {
    const ready = run(buildContactsReadyScript(), { timeout: 10000, appName: "Contacts" });
    last = ready;
    if (ready.ok && contactsAppIsReady(ready.output)) {
      return { ok: true, kind: null };
    }
    if (!ready.ok && (ready.kind === "tcc" || ready.kind === "timeout")) {
      return ready;
    }
    if (i < tries - 1) sleep(intervalMs);
  }

 ...
 *
 * Plain compose (`mail_send` / `mail_draft`) must not set AppleScript
 * `content` on `make new outgoing message`. That property, and a later
 * `set content` / `set html content`, all route through Mail's share-insert
 * path and serialize as a citation. Mail's `mailto` command returns the
 * outgoing message so we can still add recipients, hide the window, and
 * send or save — without Accessibility / System Events, and without a
 * quote-wrapped body.
 *
 * `body_format: "html"` still sets `html content` after mailto (Mail may
 * wrap that HTML part; the default plain path does not set it).
 */
export function buildComposeScript({ to, cc, bcc, subject, body, send, html = false }) {
  ...
  const plainBody = html ? stripHtmlTags(body) : body;
  const mailtoUrl = buildMailtoComposeUrl({ subject, body: plainBody });

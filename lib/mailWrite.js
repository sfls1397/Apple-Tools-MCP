...
// even start composing (locating the original by Message-ID, opening it),
// which an attempts=12/retryMs=3000 budget (~36s of verify alone) pushed
// over the 60s client ceiling on a real reply/forward even though the send
// itself always completed -- trimmed further to leave real margin for that
// extra overhead, not just mail_send's own timing.
export const SENT_VERIFY_ATTEMPTS = 7;
export const SENT_VERIFY_RETRY_MS = 2500;

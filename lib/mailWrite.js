...
/**
 * Look in Sent / Outbox for a reply or forward of `messageId` (In-Reply-To
 * / References). Used after a send hang so a delivered reply is success.
 */
    ...
    try
      ...
      repeat with msg in recentMsgs
        try
          ...
          if src contains "References:" then
            if src contains ${needleAngle} then return "${SENT_VERIFY_FOUND}"
            if src contains ${needleBare} then return "${SENT_VERIFY_FOUND}"
          end if
        end try
      end repeat
    end try
 ...
 */
export function recoverIfInSent({ inReplyTo = null, subject = null } = {}) {
  const script = inReplyTo
...
function finalizeMailWrite({
  ...
  if (sendNow && isMailSendTimeout(result)) {
    const recovered = recoverIfInSent({ inReplyTo, subject });
    if (recovered) {
      return {
        ok: true,
        recovered: true,

 ...
 * fail. ETIMEDOUT / -1712 is a timeout; -1743 / -10004 is a hard deny.
 */
 ...
 * message` path does not, so also match a recent "Re: " + original subject.
 */
      ...
      repeat with msg in recentMsgs
        ...
        try
          if origSubject is not "" then
            ...
            if subj starts with "Re:" and subj contains origSubject then return "${SENT_VERIFY_FOUND}"
          end if
        end try
      end repeat
    end try
 ...
 * replies. Match Fwd:/Fw: subject, "Begin forwarded message", original id
 * in the forwarded source, and the intended recipient. Skip In-Reply-To hits
 * so a recent reply is not counted as this forward.
 */
      ...
      repeat with msg in recentMsgs
        try
          ...
          if src contains ("In-Reply-To: " & ${needleAngle}) or src contains ("In-Reply-To: " & ${needleBare}) then
            ...
            set mentionsOrig to false
            if src contains ${needleAngle} then set mentionsOrig to true
            if src contains ${needleBare} then set mentionsOrig to true
            if origSubject is not "" and subj contains origSubject then set mentionsOrig to true
            set hitTo to false
            try
            ...
            if looksForward and (mentionsOrig or hitTo) then return "${SENT_VERIFY_FOUND}"
            if (not looksForward) and mentionsOrig and hitTo then
              if subj does not start with "Re:" then return "${SENT_VERIFY_FOUND}"
            end if
          end if
        end try
      end repeat
...
function failure(action, summary, result, secrets, { afterSend = false } = {}) {
  if (result.kind === "tcc" || isHardTccDenial(result && result.error)) {
    return `${action} failed — attempted to ${summary}. ${MAIL_TCC_GUIDANCE}`;
  }
  if (result.kind === "timeout") {
    const guidance = afterSend ? MAIL_SEND_TIMEOUT_GUIDANCE : MAIL_TCC_GUIDANCE;
    return `${action} failed — attempted to ${summary}. ${guidance}`;
  }
...
function finalizeMailWrite({
  ...
  if (sendNow && isMailSendTimeout(result)) {
    ...
    return { ok: false, message: failure(action, summary, result, secrets, { afterSend: true }) };
  }

  return { ok: false, message: failure(action, summary, result, secrets, { afterSend: false }) };
}

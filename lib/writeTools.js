...
import { messagesSend, probeMessagesAutomation } from "./messagesWrite.js";
import {
...
export const WRITE_TOOL_HANDLERS = {
  ...
  // Setup-only: live Mail/Messages Apple Events that dry_run never performs.
  // Not listed in WRITE_TOOL_DEFINITIONS, so MCP clients never see them.
  mail_automation_probe: () => probeMailAutomation(),
  messages_automation_probe: () => probeMessagesAutomation()
};

/** Public MCP write tools. Automation probes are internal to smoke / ship-gate. */
export const INTERNAL_WRITE_TOOLS = ["mail_automation_probe", "messages_automation_probe"];

export const WRITE_TOOL_NAMES = Object.keys(WRITE_TOOL_HANDLERS).filter(
  (name) => !INTERNAL_WRITE_TOOLS.includes(name)
);

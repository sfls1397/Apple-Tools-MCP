 ...
 * removed by renaming the lock aside, verifying that inode, then deleting
 * the verified temp file so a replacement at the original path is not unlinked.
 */
 ...
 *   fsApi?: Pick<typeof fs, "existsSync" | "readFileSync" | "writeFileSync" | "unlinkSync" | "mkdirSync" | "renameSync">,
 *   log?: (msg: string) => void,
...
export function createIndexerLock(options) {
  ...
  /**
   * Move indexer.lock aside, verify THAT inode's contents, then delete the
   * verified temp file. Never unlink the original path after a non-atomic
   * compare — a peer can replace indexer.lock in that gap.
   */
  function unlinkVerifiedStale(expected) {
    const tmp = `${lockFile}.stale-${pid}-${process.hrtime.bigint()}`;
    try {
      fsApi.renameSync(lockFile, tmp);
    } catch (err) {
    ...
    let got;
    try {
      got = fsApi.readFileSync(tmp, "utf8");
    } catch (err) {
    ...
    if (got !== expected) {
      try {
        fsApi.renameSync(tmp, lockFile);
      } catch (err) {
        log(`Lock restore error: ${err.message}`);
      }
      return false;
    }
  ...
  function acquire() {
    try {
      ...
      if (fsApi.existsSync(lockFile)) {
        ...
        try {
          if (!unlinkVerifiedStale(lockData)) {
            ownsLock = false;
            return false;
          }

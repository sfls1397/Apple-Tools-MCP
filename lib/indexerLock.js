...
export function createIndexerLock(options) {
  ...
  function tryAcquireMutex() {
    ...
    try {
      fsApi.writeFileSync(mutexPath, token, { flag: "wx" });
      return true;
    } catch (err) {
    ...
    try {
      const data = fsApi.readFileSync(mutexPath, "utf8");
      const parsed = parseLockData(data);
      if (parsed && isAlive(parsed.pid)) {
        return false;
      }
      const again = fsApi.existsSync(mutexPath) ? fsApi.readFileSync(mutexPath, "utf8") : null;
      if (again !== data) {
        return false;
      }
      fsApi.unlinkSync(mutexPath);
      fsApi.writeFileSync(mutexPath, token, { flag: "wx" });
      return true;
    } catch (err) {
      if (err && err.code === "EEXIST") {

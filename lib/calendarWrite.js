...
export function parseEventKitAddOutput(output) {
  ...
  const [externalId, eventKitId, calendar] = text.split("<<>>");
  const ext = String(externalId || "").trim();
  const local = String(eventKitId || "").trim();
  return {
    eventId: ext || local || null,
    eventKitId: local || null,
...
function addRow(c) {
  if (!c) return;
  var name = "";
  try { name = String(c.title); } catch (e) { return; }
...
var store = $.EKEventStore.alloc.init;
function titleOf(c) {
  try { return c ? String(c.title || "") : ""; } catch (e) { return ""; }
}
var defaultCal = null;
...
for (var i = 0; i < count; i++) {
  ...
  if (titleOf(c).toLowerCase() === wanted && writable) {
    target = c;
    break;
  }
}
if (!target && defaultCal && titleOf(defaultCal).toLowerCase() === wanted) {
  target = defaultCal;
}
if (!target && defaultCal && count === 0) {
  target = defaultCal;
}
if (!target) {
  throw new Error("CALENDAR_NOT_FOUND status=" + status + " eventKitCalendars=" + count + " default=" + titleOf(defaultCal));
}
var event = null;
...
if (!externalId && !localId) throw new Error("EVENTKIT_NO_ID status=" + status);
externalId + "<<>>" + localId + "<<>>" + titleOf(target);`;
}

...
function failure(action, summary, result, secrets = []) {
  ...
  if (result.kind === "app_unavailable") {
    return `${action} failed — attempted to ${summary}. Calendar.app is not available on this host.`;
  }
...
export function calendarAdd(args = {}) {
  ...
  const end = args.end
    ...
    : { parts: { ...start.parts, hour: allDay ? 0 : Math.min(start.parts.hour + 1, 23) }, error: null };
...
export function calendarRemove(args = {}) {
  ...
  const result = runAppleScript(buildRemoveEventScript(eventId), { timeout: 60000 });

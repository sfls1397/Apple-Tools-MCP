...
/**
 * Delete the event inside the calendar that owns it.
 *
 * Do not find the event in a handler and `delete theEvent` in a later
 * `tell application "Calendar"` — that detached specifier is the Mini
 * ship-gate failure: add/edit succeed, delete returns -1728 "Can't get
 * event", which was misread as Calendar.app unreachable.
 */
...
${scoped}
  repeat with cal in targetCals
    try
      tell cal
        if (count of (every event whose uid is ${uidLit})) > 0 then
          set removedTitle to summary of item 1 of (every event whose uid is ${uidLit})
          try
            delete (every event whose uid is ${uidLit})
          on error
            delete item 1 of (every event whose uid is ${uidLit})
          end try
          return removedTitle
        end if
      end tell
...
export function calendarRemove(args = {}) {
  ...
  const result = runAppleScript(buildRemoveEventScript(eventId, { calendarName }), { timeout: 60000, appName: "Calendar" });

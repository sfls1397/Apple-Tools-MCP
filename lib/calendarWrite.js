...
export function describeEventKitWriteFailure(action, summary, eventKitResult) {
  ...
  const head = raw.includes("EVENTKIT_NOT_FOUND")
    ? `${action} failed — attempted to ${summary}. EventKit could not find that eventkit_id (tried eventWithIdentifier and calendarItemExternalIdentifier, including colon-split halves). Calendar.app uid lookup was skipped to avoid an iCloud hang.`
...
var verify = findEventByIds(store, [localId, itemId, externalId].filter(Boolean));
if (!verify) {
  throw new Error("EVENTKIT_LOOKUP_FAILED status=" + status + " eventkit=" + localId + " item=" + itemId + " external=" + externalId);
}

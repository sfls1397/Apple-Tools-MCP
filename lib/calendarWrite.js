...
/**
 * EventKit fallback after Calendar.app `delete` fails. Same iCal UID
 * calendar_add returns. Uses Calendars privacy (not Apple Events), so it
 * can succeed when Automation→Calendar.app delete hangs on iCloud.
 */
export function shouldTryEventKitRemove(result) {
  if (!result || result.ok) return false;
  return !String(result.error || "").includes("CALENDAR_NOT_FOUND");
}

export function buildEventKitRemoveScript(eventId) {
  const uidLit = JSON.stringify(String(eventId));
...
var items = store.calendarItemsWithExternalIdentifier(uid);
if (!items || items.count === 0) {
  throw new Error("EVENTKIT_NOT_FOUND status=" + status);
}
var removed = 0;
for (var i = 0; i < items.count; i++) {
  var item = items.objectAtIndex(i);
  if (!item.isKindOfClass($.EKEvent)) continue;
  var err = Ref();
  var ok = store.removeEventSpanCommitError(item, $.EKSpanThisEvent, true, err);
  if (!ok) {
    var desc = "unknown";
...
export function calendarRemove(args = {}) {
  ...
  // Same write-bridge RPC as calendar_add / calendar_edit: { tool, args }
  // into launchd-owned node. 25s spawnSync so a hung iCloud delete cannot
  // sit for 60s and look like TCC; AppleScript's own timeout is 20s.
  ...
  let eventKitResult = null;
  if (shouldTryEventKitRemove(result)) {
    eventKitResult = runAppleScript(buildEventKitRemoveScript(eventId), {
      timeout: 20000,
      appName: "Calendar",
      language: "JavaScript"
    });

...
describe('mail_send', () => {
  it('sends to a single recipient and reports what happened only after Sent verify', () => {
    ...
    expect(script.indexOf('set focused of atmBody to true', fillAt)).toBeGreaterThan(fillAt)
    expect(script.indexOf('BODY_FOCUS_UNPROVEN', fillAt)).toBeGreaterThan(script.indexOf('set focused of atmBody to true', fillAt))
    expect(script.indexOf('keystroke "a" using command down', fillAt)).toBeGreaterThan(script.indexOf('BODY_FOCUS_UNPROVEN', fillAt))
    expect(script.indexOf('set the clipboard to bodyText', fillAt)).toBeGreaterThan(script.indexOf('keystroke "a" using command down', fillAt))
    expect(script.indexOf('keystroke "v" using command down', fillAt)).toBeGreaterThan(script.indexOf('set the clipboard to bodyText', fillAt))
  ...
  it('aborts without sending when focus proof does not match the body', () => {
    throwAfterSeProbe(BODY_FOCUS_UNPROVEN_SENTINEL)
    const result = mailCompose({ to: ['a@example.com'], subject: 'Hi', body: 'Hello' })
    expect(result.ok).toBe(false)
    expect(result.delivered).toBe(false)
    expect(result.planned).toBeUndefined()
    expect(result.mailbox).toBeNull()
    expect(isMailBodyFocusUnproven({ error: BODY_FOCUS_UNPROVEN_SENTINEL })).toBe(true)
    expect(isMailBodyFocusFailed({ error: BODY_FOCUS_UNPROVEN_SENTINEL })).toBe(false)
    expect(isMailBodyPasteMisdirected({ error: BODY_FOCUS_UNPROVEN_SENTINEL })).toBe(false)
    expect(result.message).toContain('focus proof failed')
    expect(result.message).toContain('nothing was sent')
    expect(result.message).not.toContain('header field')
    expect(result.message).not.toContain(MAIL_TCC_GUIDANCE)
    expect(result.message).not.toContain('Hello')
    expect(osascript).toHaveBeenCalledTimes(2)
    expect(mcpWriteResult(result).isError).toBe(true)
...
describe('mail compose native paste (FB11734014, -2753)', () => {
  ...
  it('finds the body by role, proves focus, then pastes — not Mail content of', () => {
    ...
    expect(jxa).toContain('return "FOUND"')
    expect(jxa).not.toContain('AXUIElementSetAttributeValue')
    expect(jxa).not.toContain('return "OK"')
    ...
    expect(handler).toContain('on atmFocusedIsProvenBody()')
    expect(handler).toContain('on atmAxValueIsRecipientToken(atmVal)')
    ...
    expect(handler.indexOf('entire contents of atmWin', fillAt)).toBeGreaterThan(fillAt)
    expect(handler.indexOf('set focused of atmBody to true', fillAt)).toBeGreaterThan(handler.indexOf('entire contents of atmWin', fillAt))
    expect(handler.indexOf('BODY_FOCUS_UNPROVEN', fillAt)).toBeGreaterThan(handler.indexOf('set focused of atmBody to true', fillAt))
    expect(handler.indexOf('keystroke "a" using command down', fillAt)).toBeGreaterThan(handler.indexOf('BODY_FOCUS_UNPROVEN', fillAt))
    expect(handler.indexOf('keystroke "v" using command down', fillAt)).toBeGreaterThan(handler.indexOf('keystroke "a" using command down', fillAt))
    ...
    expect(script).toContain('atmFocusedIsProvenBody')
    expect(script).not.toContain('atmAxHitFill')
    expect(script).not.toContain('AXUIElementSetAttributeValue')
    ...
    expect(script).not.toMatch(/content:/)
    const gateAt = script.indexOf('if my atmFocusedIsProvenBody()')
    expect(gateAt).toBeGreaterThan(-1)
    expect(script.indexOf('atmAxValueIsRecipientToken', gateAt)).toBeGreaterThan(gateAt)
  ...
  it('title-matches the compose window then focuses the AXWebArea before paste', () => {
    ...
    expect(handler).toContain('atmTopElemExactTitle')
    ...
    const fillAt = handler.indexOf('on atmFillMailBody')
    expect(fillAt).toBeGreaterThan(-1)
    expect(handler.indexOf('set focused of atmBody to true', fillAt)).toBeGreaterThan(fillAt)
    expect(handler.indexOf('if atmFocRole is not "AXWebArea" then error "BODY_FOCUS_UNPROVEN"', fillAt)).toBeGreaterThan(handler.indexOf('set focused of atmBody to true', fillAt))
    expect(handler.indexOf('keystroke "v" using command down', fillAt)).toBeGreaterThan(handler.indexOf('if atmFocRole is not "AXWebArea" then error "BODY_FOCUS_UNPROVEN"', fillAt))
  })
  ...
  it('wraps AXTitle reads inside System Events tell (2.1.4 Mini osacompile -2741)', () => {
    ...
    expect(script).toContain('atmTopElemExactTitle')
    expect(script).toContain('entire contents')
    expect(script).toContain('set focused of atmBody to true')
    expect(script).toContain('BODY_FOCUS_FAILED')
    expect(script).toContain('BODY_FOCUS_UNPROVEN')
  ...
  it('does not inject quote prefixes or a cite-blockquote into a plain compose', () => {
    ...
    expect(script).toContain('character id 65532')
    expect(script).toContain('keystroke "v" using command down')
    expect(script).not.toMatch(/content of newMessage/)
    expect(script).not.toMatch(/content of msg/)
    expect(script).not.toMatch(/\bfocused UI element\b/)
  ...
  it('does not wrap a plain-looking HTML compose in blockquote or > prefixes', () => {
    const htmlBody = '<p>Quick note about your Mac</p>\n<p>Second line</p>'
    expect(composeNativeBody(htmlBody, { html: true })).toBe('Quick note about your Mac Second line')
    expect(composeNativeBody(htmlBody, { html: true })).not.toMatch(/^>/)
    expect(composeNativeBody(htmlBody, { html: true })).not.toContain('<blockquote')
    ...
    expect(script).not.toContain('> Quick note about your Mac')
    expect(script).not.toMatch(/(^|\n)>[ \t]/)
    expect(script).not.toContain('<p>Quick note about your Mac</p>')
    expect(script).not.toMatch(/set newMessage to mailto/)
  })
  ...
  it('tells users how mail sends work, without a version history', () => {
    ...
    expect(readme).toContain('Outbox')
    expect(readme).toContain('planned: true')
    expect(readme).toContain('dry_run')
    expect(readme).not.toMatch(/2\.\d+\.\d+/)
    expect(readme).not.toContain('FB11734014')
    expect(readme).not.toContain('Mac Mini')
    expect(readme).not.toContain('MacBook')
  })
...
describe('write smoke script routing (ship gate)', () => {
  ...
  it('does not claim a dry run changes nothing at all', async () => {
    ...
    expect(readme).toContain('dry_run')
    expect(readme).not.toMatch(/dry run changes nothing at all/i)
  })

  it('documents the permissions a user has to grant, not a Contacts/Calendars + button', () => {
    const readme = fs.readFileSync(path.join(root, 'README.md'), 'utf8')
    ...
    expect(readme).toContain('Do not add node with the **+** button')
    expect(readme).toContain('often have no Add button')
    expect(readme).not.toMatch(/Grok|anysphere|\bsand\b/i)
    expect(readme).not.toContain('Mac Mini')
    expect(readme).not.toContain('MacBook')

    expect(readme).toContain('Mail, Messages, Contacts, Calendar, and System Events')
    expect(readme).toContain('Leave **Mail, Messages, and Contacts** running')
    ...
    expect(readme).toContain('does not talk to those apps')
    expect(readme).toContain('not "Mail could not be reached."')
    ...
    expect(readme).toContain('only prints a reminder')
